...
def get_latest_trade_date(data_dir: str) -> str:
    """自动从 data_dir 中检测最新的交易日期"""
    pattern = re.compile(r"anomaly_results(\d{4}-\d{2}-\d{2})\.csv$")
    dates = []
    ...
    logger.info(f"自动检测到最新交易日期: {latest}")
    return latest
# ═══════════════════════════════════════════════════════════════════════════════
# 主入口
# ═══════════════════════════════════════════════════════════════════════════════
def quick_lookup_option_info(stock_code, df):
    stock_data = df[df['stock_owner'] == stock_code]
...
if __name__ == "__main__":
    ...
    if not end_date or end_date == "auto":
        end_date = get_latest_trade_date(CONFIG["data_dir"])
        if not end_date:
            logger.error("无法自动检测最新日期，请手动指定 end_date")
            sys.exit(1)
    ...
    print("\n✅ 分析完成！")
    df = results['factors_contract'].copy()
    stock_data = quick_lookup_option_info('US.TSLA',df)
